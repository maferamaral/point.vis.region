...
// Retorna z-componente do produto vetorial (b-a) x (c-a)
...
bool segmentos_intersectam(Segmento s1, Segmento s2)
{
    ...
    if (fabs(d1) < EPSILON && on_segment(s1.p1, s2))
        return true;
    if (fabs(d2) < EPSILON && on_segment(s1.p2, s2))
        return true;
    if (fabs(d3) < EPSILON && on_segment(s2.p1, s1))
        return true;
    if (fabs(d4) < EPSILON && on_segment(s2.p2, s1))
        return true;
...
{
    double d1 = produto_vetorial(a, b, p);
    double d2 = produto_vetorial(b, c, p);
    double d3 = produto_vetorial(c, a, p);
